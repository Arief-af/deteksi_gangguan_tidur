...
from sklearn.linear_model import LogisticRegression  # Changed from KNeighborsClassifier
...
# --- Fungsi baru untuk membuat dan menyimpan plot feature importances (adapted for Logistic Regression coefficients) ---
def generate_and_save_feature_importance_plot(model, feature_names, save_path='static/feature_importances.png'):
    """
    Generates a bar plot of feature importances (coefficients for Logistic Regression)
    and saves it as an image file.
    ...
    try:
        ...
        # For multi-class (OvR or multinomial), model.coef_ will be a 2D array (n_classes, n_features).
        # We can take the average absolute coefficient or focus on a specific class.
        # For simplicity, we'll average the absolute coefficients across classes.
        
        if hasattr(model, 'coef_'):
            # Calculate mean absolute coefficient across classes
            ...
            plt.ylabel('Feature')
            plt.title('Feature Importances (Mean Absolute Coefficients from Logistic Regression)')
            plt.tight_layout()
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            plt.savefig(save_path)
            plt.close()
            ...
            print("Model does not have 'coef_' attribute. Cannot generate feature importance plot.")
    except Exception as e:
...
# Langkah 6: Melatih dan Men-tuning model Logistic Regression
print("\nMelakukan Hyperparameter Tuning dengan GridSearchCV untuk Logistic Regression...")
...
# Note: 'lbfgs', 'sag', 'saga' also support L2. 'saga' supports L1, L2, Elastic-Net.
# For simplicity and broad compatibility with L1/L2, 'liblinear' is often a good default choice.
# For multi-class, 'ovr' (one-vs-rest) is the default for LogisticRegression.

grid_search = GridSearchCV(estimator=LogisticRegression(random_state=RANDOM_STATE, multi_class='ovr'), # Changed model here
...
# --- Panggilan ke fungsi plot feature importances (for Logistic Regression) ---
# Ensure the model has 'coef_' attribute before calling this.
if hasattr(model, 'coef_'):
    generate_and_save_feature_importance_plot(model, X.columns.tolist(), save_path='static/feature_importances.png')
    print("Plot feature importances berhasil dibuat dan disimpan di static/feature_importances.png")
else:
    print("Feature importance plot skipped as the model does not have coefficients (e.g., if it failed to train).")

# --- Menyimpan model, scaler, label encoder, dan daftar kolom ---
print("\n--- Menyimpan model, scaler, label encoder, dan daftar kolom ---")
os.makedirs('model', exist_ok=True)
joblib.dump(model, 'model/logistic_regression_model.pkl') # Changed filename
...
# If you were running this as a script, you might not have app.py directly here.
# The app.py part is for the Flask API, which loads the saved model.