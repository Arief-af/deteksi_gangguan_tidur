...
from sklearn.neighbors import KNeighborsClassifier  # Changed from DecisionTreeClassifier
...
# This function is not applicable for KNN as it does not inherently provide feature importances.
# Keeping it here but it will not be called for KNN.
def generate_and_save_feature_importance_plot(feature_importances, feature_names, save_path='static/feature_importances.png'):
    ...
    """
    try:
        # For KNN, there are no direct feature importances like in tree-based models.
        # This function will not be called in the KNN version, but if it were,
        # you might need to adapt it or remove it.
        print("Feature importance plot generation skipped for KNN as it does not have inherent feature importances.")
    except Exception as e:
...
# Langkah 6: Melatih dan Men-tuning model K-Nearest Neighbors Classifier
print("\nMelakukan Hyperparameter Tuning dengan GridSearchCV untuk KNeighborsClassifier...")
...
grid_search = GridSearchCV(estimator=KNeighborsClassifier(), # Changed model here
...
# Opsional: Menampilkan pentingnya fitur (Feature Importance) - REMOVED FOR KNN
# KNN does not have feature importances
# print("\nPentingnya Fitur (Feature Importance) dari model terbaik:")
# feature_importances = pd.Series(model.feature_importances_, index=X.columns)
# print(feature_importances.sort_values(ascending=False))

# --- Panggilan ke fungsi plot feature importances --- REMOVED FOR KNN
...
joblib.dump(model, 'model/knn_model.pkl') # Changed filename
