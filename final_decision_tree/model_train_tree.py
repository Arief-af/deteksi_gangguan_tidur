...
# Langkah 6: Melatih dan Men-tuning model Decision Tree Classifier
print("\nMelakukan Hyperparameter Tuning dengan GridSearchCV untuk DecisionTreeClassifier...")
# Definisikan parameter grid untuk Decision Tree
param_grid = {
    'max_depth': [5, 10, 15, None], # Kedalaman maksimum pohon
    'min_samples_split': [2, 5, 10], # Minimal sampel untuk membelah node
    'min_samples_leaf': [1, 2, 4], # Minimal sampel di daun
    'criterion': ['gini', 'entropy'], # Kriteria kualitas split
    'splitter': ['best', 'random'], # Strategi pemisahan
    'class_weight': ['balanced', None] # Strategi bobot kelas
}

grid_search = GridSearchCV(estimator=DecisionTreeClassifier(random_state=RANDOM_STATE), # Changed model here
...
joblib.dump(model, 'model/decision_tree_model.pkl') # Changed filename
